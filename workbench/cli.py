...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    print("  max output tokens — enforced by REFUSING an over-long task, not")
    print("  by truncating it, which is why an earlier check called it broken")
    print("  and was itself the broken thing. The matrix records it as PASS.")
    print()
    ...
    print(f"                        {api_why[:150]}")
    print("                        Built and offline-tested: the count_tokens")
    print("                        endpoint, batch submission, and")
    print("                        temperature/top_p/top_k on models old enough to")
    print("                        accept them. Run with --backend anthropic-api")
    print("                        once a credential exists.")
    print("                        Counting tokens does NOT need that credential:")
    print("                        tools/count_tokens.py measures them through the")
    print("                        CLI by differencing usage.input_tokens, and the")
    print("                        matrix records that row as PASS. What is missing")
    print("                        is the free endpoint, not the capability.")
    print()
    print("Emulated on the CLI:")
    print("  stop_sequences — no CLI flag exists, so the backend truncates the")
    print("  answer at the first sequence and sets stop_reason='stop_sequence'.")
    print("  The visible output matches the API; the economics do not, because")
    print("  the API stops GENERATING and bills less while this discards text")
    print("  already paid for.")
    print()
    print("Not controllable:")
    print("  temperature, top_p, top_k — no CLI flag under any spelling probed.")
    print("  On models after Opus 4.6 they are rejected by the API with a 400")
    print("  regardless; on older models the anthropic-api backend sends them,")
    print("  once credentialed.")
