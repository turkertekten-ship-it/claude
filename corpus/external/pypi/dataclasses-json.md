---
date: 2024-06-09T16:20:16+0000
source: https://pypi.org/project/dataclasses-json/
---
# Dataclasses JSON
...
py to JSON table, this library supports the following:

- any arbitrary Collection type is supported.
Mapping types are encoded as JSON objects and str types as JSON strings.
...
the mixin approach when using static analysis tools (e.g. linting, typing),
but the differences in implementation will be invisible in runtime usage.
...
1. You can save them in a catch-all field and do whatever needs to be done later. Simply set the undefined
...
1. All 3 options work as well using schema().loads and schema().dumps, as long as you don't overwrite it by specifying schema(unknown=<a marshmallow value>).
marshmallow uses the same 3 keywords 'include', 'exclude', 'raise'.
1. All 3 operations work as well using __init__, e.g. UnknownAPIDump(**dump_dict) will not raise a TypeError, but write all unknown values to the field tagged as CatchAll.
...
on performance, and finishing this issue.
