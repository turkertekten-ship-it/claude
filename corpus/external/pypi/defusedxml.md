...
3 * 10 9 bytes, hence the name “billion laughs”. The resulting string
...
lookups by default.
1. libxml2 and lxml are not directly vulnerable to gzip decompression bombs
but they don’t protect you against them either.
1. xml.etree doesn’t expand entities and raises a ParserError when an entity
occurs.
1. minidom doesn’t expand entities and simply returns the unexpanded entity
verbatim.
1. genshi.input of genshi 0.6 doesn’t support entity expansion and raises a
ParserError when an entity occurs.
1. Library has (limited) XInclude support but requires an additional step to
process inclusion.
1. These are features but they may introduce exploitable holes, see
...
The defusedxml package (defusedxml on PyPI)
...
original module. Merely defusedxml.xmlrpc is implemented as monkey patch.
...
The defusedexpat package (defusedexpat on PyPI)
comes with binary extensions and a
modified expat library instead of the standard expat parser. It’s
...
hash collision attacks, thus reducing the performance to O(n 2) again.
...
DTD has more features like <!NOTATION>. I haven’t researched how
...
Information in XML DoS and Defenses (MSDN) suggest that .NET is
