---
date: 2026-03-11T02:19:36+0000
source: https://pypi.org/project/configargparse/
---
# ConfigArgParse

[image: PyPI version] [image: Supported Python versions] [image: Downloads per week] [image: Build status] [image: API Documentation]
...
1. config file and env. var settings should be processed by appending
...
1. configargparse shouldn't override argparse's
convert_arg_line_to_args method so that all argparse unit tests
can be run on configargparse.
1. in terms of what to allow for config file keys, the "dest" value of
...
1. don't force users to put config file settings in the right .ini [sections].
...
TOML config files.
1. if necessary, config-file-only args can be added later by
