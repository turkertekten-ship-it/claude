...
implements the functions from_file(filename[, mime]) and
...
environment variable PUREMAGIC_DEEPSCAN=0.
...
- Python — Validates Python source via ast.parse() and keyword analysis
...
- -m, --mime — Return the MIME type instead of file extension
- -v, --verbose — Print verbose output with all possible matches
...
- Removed puremagic.what() — The imghdr drop-in replacement
has been removed. Use puremagic.from_file() instead.
- Removed magic_header_array, magic_footer_array, and
multi_part_dict from the public API.
- Removed setup.py — The project now uses pyproject.toml
exclusively.
- Internal functions have been renamed from private (e.g. _magic_data)
to public (e.g. magic_data).
