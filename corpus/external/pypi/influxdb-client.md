---
date: 2026-01-23T09:39:37+0000
source: https://pypi.org/project/influxdb-client/
---
# influxdb-client-python

[image: CircleCI] [image: codecov] [image: CI status] [image: PyPI package] [image: Anaconda.org package] [image: Supported Python versions] [image: Documentation status] [image: Slack Status]

This repository contains the Python client library for use with InfluxDB 2.x and Flux. InfluxDB 3.x users should instead use the lightweight v3 client library.
...
1. Dictionary style mapping with keys: measurement, tags, fields and time or custom structure
1. NamedTuple
1. Data Classes
1. Pandas DataFrame
1. List of above items
1. A batching type of write also supports an Observable that produce one of an above item
...
1. influxdb_client.client.flux_table.CSVIterator which will iterate over CSV lines
1. Raw unprocessed results as a str iterator
1. Pandas DataFrame
...
use our multiprocessing example: import_data_set_multiprocessing.py for use a full capability of your hardware.
...
The only exception is batching WriteAPI (for more info see Batching) where you need to register custom callbacks to handle batch events.
...
Starting from version 1.27.0 for Python 3.7+ the influxdb-client package supports async/await based on asyncio, aiohttp and aiocsv.
You can install aiohttp and aiocsv directly:

> $python-mpipinstallinfluxdb-clientaiohttpaiocsv

or use the [async] extra:

> $python-mpipinstallinfluxdb-client[async]
...
> importasyncio
> frominfluxdb_client.client.influxdb_client_asyncimport InfluxDBClientAsync
> async defmain():
...
> importasyncio
> frominfluxdb_clientimport Point
> frominfluxdb_client.client.influxdb_client_asyncimport InfluxDBClientAsync
> async defmain():
> async with InfluxDBClientAsync(url="http://localhost:8086", token="my-token", org="my-org") as client:
...
> importasyncio
> frominfluxdb_clientimport OrganizationsService
> frominfluxdb_client.client.influxdb_client_asyncimport InfluxDBClientAsync
> async defmain():
> async with InfluxDBClientAsync(url='http://localhost:8086', token='my-token', org='my-org') as client:
...
The client uses Python's logging facility for logging the library activity. The following logger categories are
exposed:
