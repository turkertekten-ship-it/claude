...
> mprof run: running an executable, recording memory usage mprof plot: plotting one the recorded memory usage (by default,
> the last one) mprof list: listing all recorded memory usage files in a
> user-friendly way. mprof clean: removing all recorded memory usage files. mprof rm: removing specific recorded memory usage files
...
> >0 it might mean a memory leak. ~0 if 0 or near 0, the memory usage may be considered stable. <0 to be interpreted depending on the expected process memory usage patterns, also might mean that the sampling period is too small.
...
you should specify the function and its arguments in the tuple (f, args, kw). For example:
...
> ... b = [2] * (2 * 10 ** 7)
> ... del b
> ... return a For details refer: examples/reporting_file.py
...
> >>> from memory_profiler import LogFile
> >>> import sys
> >>> sys.stdout = LogFile('memory_profile_log', reportIncrementFlag=False) For details refer: examples/reporting_logger.py
...
> Q: How accurate are the results ? A: This module gets the memory consumption by querying the
...
> between runs. Q: Does it work under windows ? A: Yes, thanks to the
> psutil module.

## Support, bugs & wish list

For support, please ask your question on stack overflow and add the *memory-profiling* tag.
...
This module was written by Fabian Pedregosa
and Philippe Gervais
inspired by Robert Kern’s line profiler.

Tom added windows support and speed improvements via the
psutil module.

Victor added python3 support, bugfixes and general
cleanup.
