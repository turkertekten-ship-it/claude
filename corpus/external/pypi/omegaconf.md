---
date: 2026-06-11T05:05:09+0000
source: https://pypi.org/project/omegaconf/
---
# OmegaConf

| | Description |
| Project | [image: PyPI version][image: Downloads] [image: PyPI - Python Version] |
| Code quality | [image: CircleCI][image: Coverage Status][image: Total alerts][image: Language grade: Python] |
| Docs and support | [image: Documentation Status][image: Binder] |

OmegaConf is a hierarchical configuration system, with support for merging configurations from multiple sources (YAML config files, dataclasses/objects and CLI arguments)
providing a consistent API regardless of how the configuration was created.

## Releases

### Stable (2.3)

OmegaConf 2.3 is the current stable version.

- What's new
- Documentation
- Source code

Install with pip install --upgrade omegaconf

### Previous release (2.2)

- What's new
- Documentation
- Slides
- Source code

Install with pip install omegaconf==2.1

### Previous release (2.1)

- What's new
- Documentation
- Slides
- Source code

Install with pip install omegaconf==2.1

### Previous release (2.0)

- What's new
- Documentation
- Slides
- Source code

Install with pip install omegaconf==2.0.6

## Live tutorial

Run the live tutorial: [image: Binder]
