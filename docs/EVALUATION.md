...
20 questions, holding out 10 distinct documents as 20 per-question holdouts -
a document that contaminates two questions is held out twice and is still one
document - and that number only grows.

`evals/goldens-external.jsonl` runs against `corpus/external/pypi` - 266 PyPI
...
reports 5 of 54 questions affected, holding out 31 distinct documents as 36
