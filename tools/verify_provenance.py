...
#: Label vocabularies used in this repository. The digit in `KI-1`, `U-2`, `P4`
#: or `ADR 0002` names a thing; it does not assert a quantity, and reading it as
#: one makes the check fire on correct prose. A checker that flags legitimate
#: writing is one the next person switches off, so this is treated as a defect
#: in the check rather than as the cost of rigour.
IDENTIFIER = re.compile(
    r"\b(?:KI|U|L|G|P|ADR|RFC|ADR-)[-\s]?\d+\b|\bP\d+\b|\bADR\s+\d+\b",
    re.IGNORECASE,
)
...
def numbers_in(text: str) -> set[str]:
    ...
    Identifiers are removed first: `U-2` is the name of an unknown, not a claim
    that something equals two.
    """
    return set(NUMBER.findall(IDENTIFIER.sub(" ", text).replace(",", "")))
