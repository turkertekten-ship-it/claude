...
### Kitap da aynı hatayı yapıyor — bir kez

Elli birinci tur kitaba döndü. Beş turdur ölçülen şey aparat ve teslimatlardı;
hiç sorulmamış bir soru duruyordu: **kitap kendi bölümlerine doğru atıf yapıyor
mu?**

Yirmi bir iç atıfın hiçbiri var olmayan bir bölümü göstermiyor ve beş alt
bölüm atfının beşi de gerçek bir maddeye çözülüyor. Ama kırk beşinci ve
ellinci turlarda **benim** raporumda bulunan kusur, kitapta da bir kez var:

> *"Müvekkil bilgisi makinede kalır (işletim sözleşmesi §6)."* — §18.1

İşletim sözleşmesi kitabın **§3**'üdür; kuralları §3'ün içinde 1–11 diye
numaralanır ve sır saklama **6. kuraldır**. Kitabın kendi geleneğinde `§N`
ise bölümü gösterir (*"§12'deki kapı"*, *"§16'daki denetim"*). Sigili izleyen
okuyucu **§6 "Sınır ötesi katman"a** gider. Yanlış yere gönderilen kural,
kitabın en ağır sorumluluk kuralıdır ve maddenin konusu tam olarak müvekkil
sırrıdır.

Bu kusur **yamalanamaz**: sınanan şey kitabın kendi cümlesidir. Bu yüzden
`KITAP-ERRATA.md` §18'e yazıldı ve `beklenen.json`'da BEYANLI taban olarak
duruyor — kitaptaki cümle düzeltilirse vaka **BEKLENMEDİK GEÇİŞ** verir ve
beyanın bayatladığını söyler. Mutasyonla iki yönde de gösterildi.

**Ve ölçüt iki kez yanıldı; ikisi de aynı dersi verdi.** Önce §18.6 ile
§18.9 atıflarımı *"karşılıksız"* saydı — değillerdi: §18'in maddeleri metinde
noktadan sonra **boşluksuz** yazılmış ve çıkarıcı onları görmüyordu. Sonra
§5.1–§5.3, §13.3, §13.7, §15.1, §15.2 atıflarını da karşılıksız saydı —
kitap **iki ayrı numaralama biçimi** kullanıyor ve ölçüt yalnızca birini
tanıyordu. Yedi geçerli atıf, "düzeltilmek" üzereydi. *Bir ölçüt kırmızıya
dönünce ilk şüpheli ölçüttür* kuralı bu turda iki kez, dokuz atıfı korudu.

### Ve yeni ölçüm, eski aleti bozuk buldu

BE'nin bulduğu atıf kusurunu rapora yazar yazmaz **AW-01 kırmızıya döndü**:
kitaba atfettiğim alıntı kitapta bulunamıyordu. Alıntı doğruydu; **çıkarıcı
yanlıştı.**

Word'de yumuşak satır sonu `<w:br/>` etiketiyle yazılır ve paragraf sonundan
ayrıdır. Kitabı okuyan beş takım da yalnızca paragraf sonunu satır sonuna
çeviriyor, yumuşak satır sonunu ise öteki etiketlerle birlikte **siliyordu.**
Sonuç: satır sonunun iki yanındaki sözcükler birbirine yapışıyordu. Kitapta
**1022** yumuşak satır sonu var; çıkarılan metin 1038 karakterlik ayırıcıyı
kaybediyordu. Yani kitaba yapılan her birebir arama, bir satır sonunu geçtiği
anda **on bir tur boyunca sessizce başarısız oluyordu** — ve hiçbir vaka bunu
söylemedi, çünkü hepsi aynı bozuk metni okuyordu.

Çıkarıcı tek bir yere alındı (`sinama/kitap.py`). Aynı kusurun beş kopyası
beş kez düzeltilmeyi bekler ve dördü unutulur.

**Düzeltme üç takımı birden kırdı — ve kırması gerekiyordu.** Metin
düzelince "^N. Başlık" deseni artık bölüm başlığı ile numaralı liste
maddesini ayırt edemez oldu: ikisi de satır başında `N.` ile başlıyor. O
desen, yalnızca **bozuk metin üzerinde rastlantıyla** çalışıyordu. Yapı artık
Word'ün kendi biçem bilgisinden okunuyor: bölümler `Heading2` (yirmi tane),
alt bölümler `Heading3` (on sekiz tane), §18'in maddeleri de dokuz tane. Üçü
de kitabın söylediğiyle birebir.

**Ve D takımı bir mutasyonu "kaçtı" diye raporladı.** Kaçmamıştı: hedef aldığı
metin bu turda değişmişti, yani mutasyon **hiç inmemişti.** Kırk sekizinci ve
ellinci turların kuralı bu kez ölçüm aracının kendi içinde çıktı. `mutasyon()`
artık kum havuzunun parmak izini mutasyondan önce ve sonra alıyor; değişmemişse
sonucu **okumuyor**, `GEÇERSİZ` diyor. Bayat bir mutasyon sessiz bir kaçış
kadar tehlikelidir — ve tersi de doğrudur: hiç inmemiş bir mutasyon
*yakalandı* da görünebilir.

### Ölçüm değişti, iddia yerinde kaldı

Elli birinci tur kitabın metnini 1038 karakter değiştirdi. Kitaptan
**türetilen** her sayı o gün bozuk metin üzerinde ölçülmüştü — ama
teslimatlardaki karşılıkları kimse yeniden ölçmedi. BB bu boşluğu
kapatmıyor: BB yalnızca *"N vaka"* ve *"N takım"* sayımlarına bakar.

Ölçülünce bir sapma çıktı:

> rapor **"13 alıntı doğrulandı"** diyordu; AW takımı **21** alıntı
> doğruluyor.

Sekiz alıntı fark. Sayı, ölçtüğü şey büyürken yerinde kalmıştı. AF-05 bu işi
zaten yapıyordu — ama **elle seçilmiş iki iddia** için. BF onu bir **kayda**
çevirdi: her ölçüm iddiası, o değeri gerçekten üreten takımdan okunur ve
kayıtta karşılığı olmayan bir ölçüm kalıbı BF-02'yi kırar.

**Bir şey de öğrenildi: takımlar geçtiklerinde ölçtükleri sayıyı SAKLIYOR.**
Kanıt sütunu yalnızca vaka kırmızıyken basılıyor; oysa denetlenmesi gereken
tam olarak o sayıdır. Bu yüzden BF değeri çıktıdan değil, üreten modülün
kendisinden okuyor — yeniden hesaplayarak değil. Onuncu turun katman kuralı
burada da geçerli: **ölçtüğü şeyi kendi hesaplayan bir gözcü, iki farklı
doğru üretir ve hangisinin bozulduğu anlaşılmaz.** BF-04 bunu kalıcı kılıyor.

**Ve ölçüt dört kez yanıldı.** Biri saf hâliyle tanıdıktı: BF-04'ün yasak
listesini arayan desen, **kendi tanım satırını** buldu. Bir kusuru arayan
ölçüt, o kusuru tarif eden satırı da yakalar — bu incelemenin en sık sınıfı,
şimdi ölçütün kendi içinde. Öteki üçü: değer geçen takımın çıktısında yoktu,
kayıt karşılaştırması fazla dolambaçlıydı, ve belgeyi tek dizeye
düzleştirmek tablo satırlarını birbirine karıştırıp bir satırdaki anlatı
işaretini yandakine bulaştırıyordu — BB'de görülen tuzağın aynısı.

Turun sonunda BC de kırmızıya döndü ve **ayırıcısını düzeltmek gerekti.**
Türkçe vurgu ile İngilizce terimi ayıran kural *"küçük harfli hâli belgede
geçiyorsa vurgudur"* diyordu. Bu kural `JSON`'u Türkçe sanıyordu: sözcük
belgede küçük harfle de geçiyor — ama **kendisi olarak.** Ayırıcı yeniden
yazıldı ve şimdi **çekimli akraba** arıyor: Türkçe bir sözcük düzyazıda
*saklama / saklanabilir*, *dayanaksız / dayanan* gibi çekimlenmiş hâlleriyle
...
### En ağır kuralın yarısı hiç çalıştırılmamıştı

Kural 6 sistemin en yüksek bedelli kuralıdır. Sır kapısının iki ayağı var:
**desen** ayağı (kod adı, `A.Ş.`, dosya yolu) ve **ad kaydı** ayağı
(`hafiza/muvekkil-adlari.txt`). B takımı deseni sınıyor. Ad kaydı ayağı
**hiç sınanmamıştı** — çünkü kayıt kurulumda **boş** gelir ve boş bir kayıtla
o ayağın kodu hiç çalışmaz. Yani en ağır kuralın yarısı, altmış üç tur
boyunca ölçülmemiş durdu.

Dolu bir kayıtla ölçüldü ve bir kaçak çıktı:

> kayıtta **"Işık Holding"** · dışarı giden metinde **"Isik Holding"** →
> kapı **sessizce geçiriyor**

`ş→s` bir büyük/küçük varyantı **değildir**, bu yüzden `re.I` onu yakalayamaz;
homoglif tablosu da yalnızca Kiril ve Yunan harflerini katlıyordu. Oysa bir
Türk müvekkil adını **aksansız** yazmak Türkçe metinde en sık yapılan şeydir
— hele bir arama kutusuna yapıştırılırken, yani **tam olarak bu kapının
koruduğu yolda.**

Katlama eklendi ve sekiz yazım varyantının sekizi de yakalanıyor: birebir,
tümü büyük, noktasız `I`, aksansız, aksansız+büyük, satır kaydırmalı, URL (web adresi) kodlu. Yanlış pozitif yönü de sınanıyor — kayıtta olmayan bir ad ve kurum
adları **bloklanmıyor**, çünkü doğru işi bloklayan bir kapı bir gün içinde
kapatılır.

**Ve öz-sınama artık kapsanmayan kuralı söylüyor.** Kayıt boşken satır şöyle
bitiyor: *ad kaydı boş — kural 6'nın kayıt ayağı SINANMADI.* Kuralın kendi
buyruğu buydu: eksik olduğunu bilmek, eksik olduğunu bilmemekten iyidir.

Takım kaydı **kum havuzunda** kurar ve canlı dosyaya hiç dokunmaz — B-34'ün
dersi: o fixture bir zamanlar canlı müvekkil kaydını yok etmişti. BN-05 bunu
her koşumda doğruluyor.

**Bir yan etki de anında görüldü:** öz-sınamanın çıktı biçimi değişince ölçüm
kaydı (BF) kırmızıya döndü — deseni `vaka)` kapanışına bağlıydı. Düzeltildi:
**bir ölçütü çıktının biçimine değil, taşıdığı sayıya bağlamak gerekir.**
Ve tam koşumda **ikinci** bir gözcü aynı sebeple kırıldı: kapı kapsaması da
sayıyı kapanış parantezinden okuyordu. Tek bir biçim değişikliği iki ölçütü
birden düşürdü — **biçime bağlanan her ölçüt, ölçtüğü şeyin görünüşüne
bağlanmıştır.**
