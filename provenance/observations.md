...
## Observed — a directive layer with the shape but not the wiring

- A fourth third-party DOE implementation splits its directive layer the way this repository splits its instruction file: one `global.md` carrying cross-cutting principles, error handling and logging, plus one directive per automation carrying that automation's goal, guardrails and output format. [src:WILSON-DIRECTIVES-UNWIRED-2026-08-28]
- No Python file in it contains the word "directive"; the only cross-reference is one line of prose in an agent file pointing at the output format defined in another prose file. [src:WILSON-DIRECTIVES-UNWIRED-2026-08-28]
- Of the five guardrails in its email directive, exactly one has a corresponding line in code — `archive_noise`, the optional one. The four hard rules ("Never delete emails", "Never send replies", "Never mark emails as read", "Never move emails to trash or spam") hold because no skill calls those Gmail methods, and in the document they read exactly like the one that is wired. [src:WILSON-DIRECTIVES-UNWIRED-2026-08-28]
- Before this loop, `learn_rule --enforced-by` checked that the named guard exists, and nothing checked that any test fires it. Five rules named a guard; all five were already exercised by the suite, so the invariant added here found nothing to report — it holds the property rather than repairing it. [src:WILSON-DIRECTIVES-UNWIRED-2026-08-28]
- The same repository's debugging rules, written by its owner from their own corrections, put a threshold on repeated fixes: "after 2 failed attempts at the same strategy, step back, add more logging, and verify assumptions", preceded by "add logging first before changing code; identify the exact failure point from logs before attempting fixes". [src:WILSON-DEBUG-RULES-2026-08-28]

## Observed — the corpus this container can actually reach

- The chat index holds 11 conversations, all produced inside this container: one Claude Code session and ten subagents, spanning 36 minutes of 2026-08-27. None of the owner's own conversations are in it. [src:DELEGATION-HABITS-2026-08-28]
- Of its 433 user turns, 421 are tool results — the transcript format stores a `tool_result` block in the user role — and `prompt_habits.py` already excludes them. The 10 it scores are the prompts this session sent to its own subagents. [src:DELEGATION-HABITS-2026-08-28]
- All 10 are missing an acceptance test; 5 are missing context; the mean score is 90.6. The single most expensive habit in the only corpus available is therefore this agent's own, not the owner's. [src:DELEGATION-HABITS-2026-08-28]
- One of those 10 asked whether Saraev published anything called CLEAR and came back reporting no evidence — the negative that a single clone later overturned. It stated no acceptance test, so there was no written standard a negative had to meet. [src:DELEGATION-HABITS-2026-08-28] [src:SARAEV-REPOS-2026-08-27]
- The owner's Google Drive contains no file whose title matches `conversations`, `chat`, `prompt` or `claude`, so the corpus the request names is not reachable by that route either. Nothing was opened or downloaded. [src:DRIVE-NO-CHAT-EXPORT-2026-08-28]
